...
def init(capacity):
    global cap 
    cap = capacity
    node_for_freq.append(0)# For the frequency == 0 : USELESS
    node_for_freq.append([]) # For the frequency == 1

def _update( key, value, freq):
    # increment the frequency
    global node_for_key, node_for_freq, least_freq
    # remove the key from the old frequency
    node_for_freq[freq].remove(key)

    if len(node_for_freq[least_freq]) == 0:
        # update the least_freq if there is no more item in this frequency list
        # node_for_freq.pop(least_freq) # remove this empty freq list, to save memory
        least_freq += 1
    
    # update frequency
    node_for_key[key][1] = freq + 1
    # use a +1 because the idx 0 is not used 
    if ((freq + 1) == len(node_for_freq)):
        node_for_freq.append([])
    node_for_freq[freq + 1].append(key)
...
def set( key, value):
    ...
    if (len(node_for_key) >= cap):
        # evict 1 item
        key_to_remove = node_for_freq[least_freq].pop(0)
        node_for_key.pop(key_to_remove)

    # Insert the new item
    node_for_key[key] = [value, 1]
    node_for_freq[1].append(key)
