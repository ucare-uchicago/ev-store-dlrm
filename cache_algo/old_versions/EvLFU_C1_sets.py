...
def init(capacity):
    ...
    i = 0
    while (i <= 26):
        # store the keys in set()
        lists_C1[i] = {"dummyKey"} # Must initialize with non NULL
        lists_C1[i].remove("dummyKey") # remove the dummy key
        i += 1
...
def set(key, value, agg_hit):
    ...
    if n_perfect_item_C1 >= max_perfect_item_C1:
        ...
        if len(vals_C1) >= cap_C1:
            ...
            while(len(lists_C1[min_C1]) == 0):
                # find the right key to pop
                # Update minimum agg_hit
                min_C1 += 1
                if (min_C1 > 26):
                    min_C1 = 1
            key_to_evict = lists_C1[min_C1].pop()
            vals_C1.pop(key_to_evict)

    # insert the new value:
    vals_C1[key] = [value, agg_hit]
    lists_C1[agg_hit].add(key) # ========
