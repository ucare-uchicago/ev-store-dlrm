...
class CriteoDataset(Dataset):
    ...
    def __init__(
            ...
            percent_data_for_inference=0.008, #percent; 100% will use all dataset for inference
    ):
        data_input_name = list(filter(None, raw_path.split("/")))[-2]
        ...
        if dataset == "kaggle":
            days = 7
            out_file = "kaggleAdDisplayChallenge_processed" if (data_input_name == "") else data_input_name
        elif dataset == "terabyte":
            days = 24
            out_file = "terabyte_processed"  if (data_input_name == "") else data_input_name
        else:
        ...
        if data_ready:
            print("\nReading pre-processed data=%s" % (str(pro_data)))
            file = str(pro_data)
        else:
        ...
        # setup data
        if use_memory_map:
            print("===========     USE memory map!!!!   ==============")
            # Reduce the test_size here to configure the length of the inference test
            ...
            if split == 'none' or split == 'train':
                ...
                if (is_inference_only) :
                    # no need to split dataset into val and test
                    # dataset for testing is the whole day dataset
                    print("mmap:: use all dataset for test since we do inference only")
                    self.test_size = int(np.ceil(num_samples))
                else:
            ...
            if (is_inference_only) :
                self.test_size = int(days * self.test_size * percent_data_for_inference)
                print("mmap:: Reduce the amount of test case based on percent_data_for_inference")
                print("mmap:: self.test_size = " + str(self.test_size))
            ...
            print("mmap:: Sparse features= %d, Dense features= %d" % (self.n_emb, self.m_den))
            print("mmap:: Only a SINGLE day is used for testing out of 7 days")
            ...
            if self.split == 'test' or self.split == 'val':
                ...
                with np.load(fi) as data:
                    ...
                    # len(self.y) here is 714285 because there are 7 parts (days) == 5mil/7 
                    print("mmap:: data y = " + str(len(self.y)))

        else:
            # print("(This process eat up 1 GB)")
            print("file = " + str(file))
            # load and preprocess data [THIS NEEDS >10 GB!!]
            data = np.load(file) # retain the high memory usage!!
            # this X_int X_cat and y are holding full dataset!!! will be 5mil when using 5mil dataset!
            X_int = data["X_int"]  # continuous  feature
            X_cat = data["X_cat"]  # categorical feature
            y = data["y"]          # target
            self.counts = data["counts"]  # the unique values of each EV table
            if (is_inference_only) :
                # len(y) here is 5mil instead of 714285
                X_int, X_cat, y = reduceDatasetSize(X_int, X_cat, y, percent_data_for_inference)
            ...
            print("Is the RAM usage still high?? NOPE")
            # time.sleep(10)
            ...
            if split == "none":
                ...
                if (not is_inference_only):
                  # during the training, split this (default dlrm)
                  indices = np.array_split(indices, self.offset_per_file[1:-1])
                else:
                  # during the inference, use ALL indices
                  indices = [indices]
                print("len indices " + str(len(indices)))
                # There are 7 indeces, the first 6 is for training, and the last one is for testing and validation!

                if (is_inference_only):
                  # inference workload doesn't need any randomization! (to save memory)
                  pass
                else:
                ...
                print("Defined split = %s indices..." % (split))
                ...
                if split == 'train':
                    ...
                    print("==== train_indices = " + str(len(train_indices)))
                    ...
                    print("==== val_indices = " + str(len(val_indices)))
                    ...
                    if (is_inference_only):
                        test_indices = indices[-1]
                        print("==== Not splitting the indices, use ALL!")
                        print("==== test_indices = " + str(len(test_indices)) + "   Should be reduced because only 40k will be used!")
                        self.X_int = [X_int[i] for i in test_indices]
                        self.X_cat = [X_cat[i] for i in test_indices]
                        self.y = [y[i] for i in test_indices]
                    else:
                        test_indices = indices[-1]
                        test_indices, val_indices = np.array_split(test_indices, 2) # splitting test_indices into 2 equal array!!!

                        print("==== test_indices = " + str(len(test_indices)) + "   Should be reduced because only 40k will be used!")
                        self.X_int = [X_int[i] for i in test_indices]
                        self.X_cat = [X_cat[i] for i in test_indices]
                        self.y = [y[i] for i in test_indices]
                # empty out the memory heavy variables
                # none
            print("Is the RAM usage still high?? NOPE")
            # time.sleep(10)
        print("Is the RAM usage still high?? NOPE")
...
def reduceDatasetSize(X_int, X_cat, y, percent_data_for_inference):
    dataset_length = int(percent_data_for_inference * len(y))
    print("Reduce the amount of data loaded! dataset_length = " + str(dataset_length))
    X_int = np.array(X_int[: dataset_length])  # continuous  feature
    X_cat = np.array(X_cat[: dataset_length])  # categorical feature
    y = np.array(y[: dataset_length])          # target
    return X_int, X_cat, y
...
def make_criteo_data_and_loaders(args, offset_to_length_converter=False):
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> START make_criteo_data_and_loaders")
    print(">>>> IS INFERENCE ONLY == " + str(args.inference_only))
    if args.mlperf_logging and args.use_memory_map and args.data_set == "terabyte":
        print("DLRM+EV-Store: ERROR: Doesn't support terabyte data loader yet.")
        exit()
        ...
        print(">>>> Loading Criteo kaggle dataset!")
        train_data = []
        if (not args.inference_only):
          ...
          train_data = CriteoDataset(
              ...
              args.use_memory_map,
              args.dataset_multiprocessing,
              False, # training (not inference_only)
          )
        ...
        if (not args.inference_only):
            ...
            print(">>>>     Loaded data for testing is LIMITED TO Reduce memory during inference test!")
            test_data = CriteoDataset(
                args.data_set,
                args.max_ind_range,
                args.data_sub_sample_rate,
                ...
                args.use_memory_map,
                args.dataset_multiprocessing,
                args.inference_only, # TRUE: (inference_only)
                args.percent_data_for_inference, # 0.008% * 5mil = 40k
            )
        ...
        train_loader = []
        if (not args.inference_only):
            print(">>>>     PyTorch Preparing train_loader for TRAINing")
            train_loader = torch.utils.data.DataLoader(
                train_data,
                batch_size=args.mini_batch_size,
                shuffle=False,
        ...
        print(">>>>     PyTorch Preparing test_loader for TESTing")
        test_loader = torch.utils.data.DataLoader(
            test_data,
            batch_size=args.test_mini_batch_size,
            shuffle=False,
    ...
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> FINISH make_criteo_data_and_loaders")
