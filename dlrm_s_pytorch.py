...
def inference(
    ...
    n_progress_indicator = 40
    progress_bar_freq = int(len(test_ld) / n_progress_indicator) if (args.inference_only) else int(nbatches/ n_progress_indicator) 
    print("==== ==== Progress bar (max: " + str(progress_bar_freq * n_progress_indicator) + ") shown below:")
    ...
    return model_metrics_dict, is_best, auc_metrics, validation_results["roc_auc"]

def create_output_folder(args, data_input_name):
  print("Create folder to store the model and ev-tables")
  for i in range (0, args.nepochs):
    Path(os.path.join(STORED_MODEL_PATH, data_input_name, "epoch-" + str(i), "ev-table")).mkdir(parents=True, exist_ok=True)
...
def run():
    ...
    parser.add_argument("--run-data-prep-only", action="store_true", default=False)
    ...
    data_input_name = list(filter(None, args.input_data.split("/")))[-1]
    if (args.use_memory_map):
        ...
        args.ev_path = args.ev_path.replace(args.input_data, args.input_data + "_mmap")
        args.input_data = args.input_data + "_mmap"
        data_input_name = data_input_name + "_mmap"
        args.memory_map = True

    args.raw_data_file = os.path.join(args.input_data, TRAIN_FILE)
    args.processed_data_file = os.path.join(args.input_data, data_input_name + ".npz")

    create_output_folder(args, data_input_name)
    ...
    if args.data_generation == "dataset":
        ...
        training_config_path = os.path.join(STORED_MODEL_PATH, data_input_name, evstore_utils.TRAINING_CONFIG_FILE)
        ...
        # Inference not reaching this code!
    ...
    if (args.run_data_prep_only):
        print("Data preparation is done! You should use GPU node for the training or inference testing.")
        exit()
    ...
    print("==== nbatches (comes from the train_ld)   = " + str(nbatches))
    ...
    with torch.autograd.profiler.profile(
        ...
        if not args.inference_only: # This is for training!!
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    if should_test:
                        ...
                        model_metrics_dict, is_best, auc_metrics, better_auc = inference(
                            args,
                            dlrm,
                            best_acc_test,
                            best_auc_test,
                        ...
                        if (
                            ...
                            curr_epoch_out_path = os.path.join(STORED_MODEL_PATH, data_input_name, "epoch-" + str(k))
                            ...
                            for key,value in model_metrics_dict["state_dict"].items():
                                ...
                                if (key[:3] == "emb"):
                                    ...
                                    with open (out_path, 'w') as f:
                                        ...
                                        count += 1
                                    # print("Save to csv index " +str(count) )
                            print("==== ==== All "+ str(count) + " EV-Tables are saved to csv at " + curr_epoch_out_path)
            ...
            print("============ Begin inference workload")
