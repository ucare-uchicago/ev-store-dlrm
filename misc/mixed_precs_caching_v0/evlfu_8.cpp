...
EVLFU_8BIT::EVLFU_8BIT(int capacity, bool init_second_layer, int secondary_precision) {
    cap_C1 = capacity;
    ...
    if (init_second_layer) {
        ...
        if (secondary_precision == 4) {
            evlfu_4bit = new EVLFU_4BIT(cap_C1 * 2, false, -1);
        } else {
...
int main8() {
    EVLFU_8BIT *evlfu_8bit = new EVLFU_8BIT(7680*2, false, -4);
    // Prepare workload
