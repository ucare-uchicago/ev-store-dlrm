...
#ifndef EVLFU_16_INCLUDED
#define EVLFU_16_INCLUDED
...
class EVLFU_16BIT{
    ...
    #define BIT_PRECISION_16BIT 16 // this could be 32bit, 16bit, 8bit, or 4bit
    // BIT_Precision_16BIT impact the size of stored data
    ...
    int TOTAL_BYTE_PER_ROW = EV_DIMENSION * BIT_PRECISION_16BIT / 8; // in Bytes
    ...
    string EV_TABLE_PATH = "/mnt/extra/ev-store-dlrm/stored_model/criteo_kaggle_all_mmap/epoch-00/ev-table-16/binary/";
    ...
    // Multithreading
    #define N_THD__READ_EVTABLE_16BIT 3 // thread::hardware_concurrency(); // 48 threads
    ...
    sem_t LOCK_THD_STATUSES[N_THD__READ_EVTABLE_16BIT];  // if the worker is available, the value will be 0
  ...
  public:
    ...
    EVLFU_16BIT(int capacity, bool init_second_layer, int secondary_precision);
    ~EVLFU_16BIT();
    ...
    void chars_buffer_to_floats_old_slow(char *buffer, float * floats);
    void chars_buffer_to_floats(char *buffer, float * floats);
    char* get_from_file(int table_id, int row_id);
    int request_to_c1_c2(vector<int>& arr_row_ids, vector<bool>& c1_arr_record_hit, 
        float * c1_c2_arr_emb_weights, int debug);
