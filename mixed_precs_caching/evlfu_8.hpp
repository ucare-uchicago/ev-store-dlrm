...
#ifndef EVLFU_8BIT_INCLUDED
#define EVLFU_8BIT_INCLUDED
...
class EVLFU_8BIT {
    ...
    #define BIT_PRECISION_8BIT 8 // this could be 32bit, 16bit, 8bit, or 4bit
    ...
    int TOTAL_BYTE_PER_ROW = EV_DIMENSION * BIT_PRECISION_8BIT / 8; // in Bytes
    ...
    string EV_TABLE_PATH = "/mnt/extra/ev-store-dlrm/stored_model/criteo_kaggle_all_mmap/epoch-00/ev-table-8/binary/";
    ...
    // Multithreading
    #define N_THD__READ_EVTABLE_8BIT 3 // thread::hardware_concurrency(); // 48 threads
    ...
    sem_t LOCK_THD_STATUSES[N_THD__READ_EVTABLE_8BIT];  // if the worker is available, the value will be 0
...
public:
    ...
    vector<Cache_data> shared_arr_c3_aprx_ev = vector<Cache_data>(N_EV_TABLE);
  
    EVLFU_8BIT(int capacity, bool init_second_layer, int secondary_precision, int init_third_layer, string size_proportion);
    ~EVLFU_8BIT();
    ...
    int request_to_c1_c2(vector<int>& arr_row_ids, vector<bool>& c1_arr_record_hit, float * c1_c2_arr_emb_weights, int debug);
    int request_to_c1_c2_c3(vector<int>& arr_row_ids, vector<bool>& c1_arr_record_hit, float * c1_c2_arr_emb_weights, int debug);
    ...
    char *request_by_key(string key);
    char *find_approximate_ev(string key);
};
#endif